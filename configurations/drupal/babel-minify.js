...
const drupalOptions = options.drupal || null;
...
if (drupalOptions) {
    if (drupalOptions.postcss.compile) {
        glob(drupalOptions.postcss.path, function (er, files) {
            if (err) throw err;
    ...
    if (drupalOptions.javascript.compile) {
        glob(drupalOptions.javascript.path, function (er, files) {
            for (let file = 0; file < files.length; file++) {
                const fileName = path.basename(files[file]);
    ...
    console.log("\x1b[41m", 'Drupal object not found in config.js file');
}
